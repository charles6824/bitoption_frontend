...
const Investment = () => {
  ...
  const tableHead = ["S/N", "Amount Invested", "Expected Amount", "Completion Date","Status", ""]
  const tableData:any = [
    {
      id: "1",
        ...
        <Table data={tableData} tableHead={tableHead}>
          {tableData.map((table: any, index: number) => (
            <tr key={index}>
              ...
              <td className={` text-[15px]  ${table.status === "Completed" ? "text-green-700":"text-yellow-400 "}` }>{table.status}</td>
            </tr>
          ))}
        </Table>
