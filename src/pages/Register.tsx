...
import { useConfirmRegMutation, useRegisterUserMutation } from "../slices/baseApiSlice";
...
const Register = () => {
  ...
  const [confirmReg, {isLoading: regLoading}] = useConfirmRegMutation()
  ...
  const verifyRegistration = async (e: any) => {
    e.preventDefault()
    try {
      const model = {
        email: email,
      ...
      const response: any = await confirmReg({data: {payload: model}}).unwrap()
    ...
    <div className="flex items-center md:gap-16  h-screen md:min-h-screen bg-[#111111]">
      ...
      {step === 2 && (
        <div className="flex flex-col ">
          ...
          {
            regLoading ? (<LoadingBtn />) : (
              <button className="w-full bg-[#fa9e1f] text-white py-4  rounded hover:bg-[#e88c15] transition mt-12" onClick={verifyRegistration}>Submit</button>
            )
          }
        </div>
      )}
